...
func StartMany(ctx context.Context, reqChan chan *protocol.Request, pool *redis.Pool, count int, logger *log.Logger) {
	for i := 0; i < count; i++ {
...
func requestWorker(ctx context.Context, reqChan chan *protocol.Request, pool *redis.Pool, logger *log.Logger) {
	for {
