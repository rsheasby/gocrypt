...
func GetRequest(ctx context.Context, pool connGetter, logger *log.Logger) (request *protocol.Request) {
	// Continously pop a request off one of the queues, retrying if IO timeout
	conn := pool.Get()
	defer conn.Close()
